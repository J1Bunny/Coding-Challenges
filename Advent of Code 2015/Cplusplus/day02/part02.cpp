...
int main(){
	...
	while (!inputFile.eof()){
		...
		if (side[0] <= side[1] && side[0] <= side[2]){
			if (side[1] <= side[2]){
				smallArea = 2 * (side[0] + side[1]);
			}
			else{
				smallArea = 2 * (side[0] + side[2]);
			}
		}

		else if (side[1] <= side[0] && side[1] <= side[2]){
			if (side[0] <= side[2]){
				smallArea = 2 * (side[1] + side[0]);
			}
			else{
				smallArea = 2 * (side[1] + side[2]);
			}
		}

		else if (side[2] <= side[1] && side[2] <= side[0]){
			if (side[1] <= side[0]){
				smallArea = 2 * (side[2] + side[1]);
			}
			else{
				smallArea = 2 * (side[2] + side[0]);
			}
		}

		//totalArea = (2 * side[0] * side[1]) + (2 * side[1] * side[2]) + (2 * side[0] * side[2]);
		totalArea =side[0] * side[1] * side[2];
